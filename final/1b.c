...
void keyboard_input(char *str){ // str: the words you type on the keyboard
    ...
    if( str[ str_len -1 ] == '\r' ){
        ...
        for(int i=0;i< str_len -1 ;i ++ ){
            ...
            if( is_first ){
                first_val = first_val * 10 + str[i] - '0';
            }
            else{
                second_val =second_val*10 + str[i] -'0';
            }
        }
...
void main(){
    ...
    while(1) {
        // Do sth in main
        double d = (double)current_delay / (double)100;

        set_LED( led_val );
        delay_if_not_changed( d , &current_delay);
